...
`}</style>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-16 pt-4 pb-20 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div data-aos="fade-up" className="flex flex-col gap-6">
          ...
          {/* Email CTA */}
          <div data-aos="fade-up" className="flex items-stretch rounded-xl overflow-hidden border border-white/15 bg-white/8 backdrop-blur-sm max-w-lg">
            <input
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder="Enter your email address"
              className="flex-1 bg-transparent px-5 py-4 text-[14px] text-white placeholder:text-white/35 outline-none"
            />
            <button onClick={handleSubmit} className="flex-shrink-0 bg-gold hover:bg-gold text-[#1a2035] font-bold text-[13.5px] px-6 py-4 transition-colors duration-200 flex items-center gap-2">
              Book a Free Call
